...
def make_agent(config):
  ...
  return Agent(obs_space, act_space, elements.Config(
      **config.agent,
      ...
      replica=config.replica,
      replicas=config.replicas,
      intrinsic_type=config.intrinsic_type,
      intr_reward_scale=config.intr_reward_scale,
      intr_val_scale=config.intr_val_scale,
      use_lpm=config.use_lpm,
      lpm_eta=config.lpm_eta,
      lpm_clip=config.lpm_clip,
      lpm_warmup=config.lpm_warmup,
  ))
...
def make_logger(config):
  ...
  for output in config.logger.outputs:
    if output == 'jsonl':
      ...
      wandb.init(project='dreamerv3', name=name, config=dict(config), dir=str(logdir))
      print(f'WandB initialized: {name}')
      def filter_wandb(x):
        data = {}
        for m in x:
          if isinstance(m, dict):
            data.update({k: v for k, v in m.items() if 'policy_' not in k})
        if data:
          wandb.log(data, step=int(step))
      outputs.append(filter_wandb)
    elif output == 'scope':
