...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    if getattr(config, 'use_lpm', False):
         ...
         # self.intr = embodied.jax.MLPHead(scalar, **config.value, name='intr') # Removed for simpleLPM
         self.lpmnorm = embodied.jax.Normalize(**config.retnorm, name='lpmnorm') # Reuse retnorm config

    self.modules = [
        self.dyn, self.enc, self.dec, self.rew, self.con, self.pol, self.val]
    if getattr(config, 'use_lpm', False):
        self.modules.extend([self.lpm, self.lpmnorm]) # Removed self.intr
    ...
    if getattr(config, 'use_lpm', False):
      scales['lpm'] = 1.0
      # scales['intr'] = 1.0 # Removed for simpleLPM
    self.scales = scales
  ...
  def loss(self, carry, obs, prevact, global_step, training):
    ...
    eta = 0.0
    intr_reward_padded = jnp.zeros_like(obs['reward'])
    ...
    if getattr(self.config, 'use_lpm', False):
        ...
        # Only update predictor every N steps to create temporal lag
        update_interval = float(getattr(self.config, 'lpm_update_interval', 20))
        ...
        # Compute Intrinsic Reward (SimpleLPM: add to reward target)
        intr_raw = delta_norm_clipped * eta
        # Pad first step (0) as we only have delta for 1..T
        intr_reward_padded = jnp.concatenate([jnp.zeros_like(intr_raw[:, :1]), intr_raw], 1)
        # Ensure shape match
        if intr_reward_padded.shape[1] > obs['reward'].shape[1]:
             intr_reward_padded = intr_reward_padded[:, :obs['reward'].shape[1]]
        elif intr_reward_padded.shape[1] < obs['reward'].shape[1]:
             intr_reward_padded = jnp.concatenate([intr_reward_padded, jnp.zeros((intr_reward_padded.shape[0], obs['reward'].shape[1] - intr_reward_padded.shape[1]))], 1)

    # Calculate Reward Loss (Standard + Intrinsic)
    losses['rew'] = self.rew(inp, 2).loss(obs['reward'] + intr_reward_padded)
    losses['con'] = self.con(inp, 2).loss(1.0 - f32(obs['is_terminal']))
...
def imag_loss(
  ...
  # LPM: Add Intrinsic Reward -> REMOVED for SimpleLPM (handled in RewardModel)
