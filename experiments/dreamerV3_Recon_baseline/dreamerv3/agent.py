...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    if getattr(config, 'use_lpm', False):
         intrinsic_type = getattr(config, 'intrinsic_type', 'lpm')
         if intrinsic_type == 'lpm':
  ...
  def loss(self, carry, obs, prevact, global_step, training):
    ...
    # LPM or Recon: Intrinsic Reward Calculation
    eta = 0.0
    ...
    if getattr(self.config, 'use_lpm', False):
        intrinsic_type = getattr(self.config, 'intrinsic_type', 'lpm')
        
        if intrinsic_type == 'lpm':
            # 1. Get current error (NLL)
            # Assuming 'image' is the key for observation reconstruction
            # losses['image'] is (B, T)
            ...
            # 2. Predict error from previous state and action
            # Target: eps_current[:, 1:] (errors at t=1..T-1)
            # Input: repfeat[:, :-1] (state at t=0..T-2) + prevact[:, 1:] (action at t=0..T-2 leading to t=1..T-1)
            # Note: prevact[:, t] is action LEADING TO state t.
            # So prevact[:, 1] is action leading to state 1. repfeat[:, 0] is state 0.
            # We want to predict error at state 1 using state 0 and action leading to 1.
            lpm_inp = sg(self.feat2tensor(concat([
                jax.tree.map(lambda x: x[:, :-1], repfeat),
                jax.tree.map(lambda x: x[:, 1:], prevact)
            ], -1)))
            
            eps_target = sg(eps_current[:, 1:])
            pred_eps = self.lpm(lpm_inp, 2).pred() # (B, T-1)
            
            # Train Error Predictor - periodic update (paper-style D_queue equivalent)
            update_interval = float(self.config.lpm_update_interval)
            should_update = (global_step % update_interval) < 1.0
            lpm_loss_raw = self.lpm(lpm_inp, 2).loss(eps_target)
            ...
            # Use update=True to update statistics
            ...
            intr_loss_raw = self.intr(lpm_inp, 2).loss(intr_target)
            ...
            losses['lpm'] *= self.config.lpm_scale if hasattr(self.config, 'lpm_scale') else 1.0 # Optional scale
        
        elif intrinsic_type == 'recon':
         # 1. Get Reconstruction Loss (as error signal)
         if 'image' not in losses:
             total_recon_loss = sum([losses[k] for k in recons.keys()])
         else:
             total_recon_loss = losses['image'] # Primary recon loss
             
         # 2. Compute Intrinsic Reward Target
         # Based on Suzuki logic: log(loss + 1e-6)
         # Using 'intr_reward_scale' from config (defaults to 1.0)
         scale = getattr(self.config, 'intr_reward_scale', 1.0)
         target_intr_rew = scale * jnp.log(total_recon_loss[:, 1:] + 1e-6)
         ...
         metrics['intr_rew_target'] = target_intr_rew.mean()
         
         # 3. Train Intrinsic Reward Model (self.intr)
         # Input: repfeat[:, :-1] (state at t-1) to predict reward at t
         # We need to construct input similar to LPM or just use state.
         # DreamerV3 Separated logic used just state. 
         # Let's align with LPM input structure for fairness if possible, OR just state.
         # For consistency with LPM above, we use (state, action).
         rep_slice = jax.tree.map(lambda x: x[:, :-1], repfeat)
         act_slice = jax.tree.map(lambda x: x[:, 1:], prevact)
         
         # Handle action logic (consistent with LPM block above)
         ...
         intr_inp = sg(jnp.concatenate([rep_tensor, act_tensor], -1))
         
         # target_intr_rew is (B, T-1)
         losses['intr'] = self.intr(intr_inp, 2).loss(sg(target_intr_rew))
    ...
    los, imgloss_out, mets = imag_loss(
        ...
        prev_inp=jnp.concatenate([
            ...
            # Robust action handling for dummy envs (act_disc/act_cont) or standard (action)
            # Use same logic as LPM input construction:
            (lambda act_slice: jnp.concatenate([
                jax.nn.one_hot(nn.cast(act_slice['action']), self.act_space['action'].high)
                if 'action' in act_slice and self.act_space['action'].discrete 
                else jnp.concatenate([v.astype(jnp.float32)[..., None] if v.ndim == 2 else v.astype(jnp.float32) for v in act_slice.values()], -1)
            ], -1))(sg(jax.tree.map(lambda x: x[:, 1:], imgact)))
...
def imag_loss(
  ...
  # Metrics for Extrinsic Reward (before adding intrinsic)
  metrics['rew_ext_mean'] = rew.mean()
  metrics['rew_ext_max'] = rew.max()
  ...
  if intr_model is not None:
      ...
      # Metrics for Intrinsic Reward
      metrics['rew_intr_mean'] = intr_padded.mean()
      metrics['rew_intr_max'] = intr_padded.max()
      metrics['rew_intr_std'] = intr_padded.std()
