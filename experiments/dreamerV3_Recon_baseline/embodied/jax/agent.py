...
class Agent(embodied.Agent):
  ...
  def __init__(self, model, obs_space, act_space, config, jaxcfg):
    ...
    self._train = transform.apply(
        ...
        (dona_sharding, allo_sharding, tm, ts, ts, tm), (tp, ts, ts, tm), ar,
    ...
    with self.train_lock:
      with elements.timer.section('jit_train'):
        with jax.profiler.StepTraceAnnotation(
          ...
          global_step = internal.device_put(
              np.array(global_step, dtype=np.float32), self.train_mirrored)
          self.params, carry, outs, mets = self._train(
              dona, allo, seed, carry, data, global_step)
  ...
  def report(self, carry, data, global_step=0.0):
    ...
    with self.train_lock:
      global_step = internal.device_put(
          np.array(global_step, dtype=np.float32), self.train_mirrored)
      carry, mets = self._report(self.params, seed, carry, data, global_step)
  ...
  def _compile_train(self):
    ...
    dona = {k: v for k, v in self.params.items() if k not in self.policy_keys}
    global_step = internal.device_put(
        np.array(0.0, dtype=np.float32), self.train_mirrored)
    self._train = self._train.lower(dona, allo, seed, carry, data, global_step).compile()
  ...
  def _compile_report(self):
    ...
    global_step = internal.device_put(
        np.array(0.0, dtype=np.float32), self.train_mirrored)
    self._report = self._report.lower(self.params, seed, carry, data, global_step).compile()
