...
`train_lpm_modified.py` で使用できるLPM関連のパラメータ解説です。次回の実験ではこれらの値を微調整することで、さらなる性能向上（早期収束の回避）が期待できます。

### 1. 必須設定
| パラメータ | デフォルト | 説明 |
| :--- | :--- | :--- |
| `--enable-lpm` | `False` | **必須**。これを付けないとLPMロジックが走りません。 |

### 2. チューニング可能なパラメータ (推奨)

最も効果が高いのは、**「成功時の探索量 (`focus-eta`)」を増やす**ことです。

| パラメータ | デフォルト | 役割・調整指針 |
| :--- | :--- | :--- |
| `--lpm-focus-eta` | `0.1` | **【重要】** 報酬獲得中（Focus Mode）にどれだけ探索を残すか。<br>- `0.1`: ほぼ探索しない（現状）。安定するが局所解に陥りやすい。<br>- `0.3`〜`0.5`: **推奨設定**。稼ぎながらも新しいルートを3〜5割の力で探し続ける。 |
| `--lpm-decay-rate` | `0.995` | Focus Modeへの移行速度。<br>- `0.995`: 急速に切り替わる（現状）。<br>- `0.999`: ゆっくり切り替わる。初期の探索期間を実質的に延ばす効果がある。 |
| `--lpm-eta` | `1.0` | 探索最大時の内発的報酬係数。<br>- 通常は `1.0` で問題ないが、もし探索が過剰で全く学習しない場合は `0.5` 程度に下げる。 |

### 次回の実験コマンド例 (案)
「早期収束」を防ぎ、よりハイスコアを目指すための設定例です。

```bash
docker exec -it lpm-dreamer python train_lpm_modified.py \
    --env-name ALE/BankHeist-v5 \
    --enable-lpm \
    --steps 200000 \
    --wandb \
    --lpm-focus-eta 0.4 \
    --lpm-decay-rate 0.999
```
