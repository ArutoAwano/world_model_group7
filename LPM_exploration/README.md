...
### 詳細: 環境探索のメカニズム (Exploration Mechanism Detail)

「LPMの仕組みはわかったが、実際にエージェントはどうやって未知の場所へ足を運んでいるのか？」
この問いに対する答えは、**「記憶（Experience）」と「夢（Dreaming）」のサイクル**にあります。

#### 1. 昼：冒険と「感動」の記録 (Interaction)
エージェントが世界を歩き回ります（物理的にはヘッドレス環境で描画された画像をEncoderで見ています）。 
ある時、偶然「予測できない場所（例：洞窟）」を見つけると、以下のプロセスが走ります。

1.  **知覚**: Encoderが画像をベクトルに変換し、RSSMが「現在の状態」を認識します。
2.  **驚き**: ErrorPredictorが「ここは予測エラーが大きい（未知だ）！」と検知します。
3.  **記録**: エージェントは「この状態 (State) になったら**報酬 (Intr_Reward) が貰えた**」という事実を **ReplayBuffer (記憶)** に書き込みます。
    *   **重要**: この時点では、エージェントはまだ「洞窟へ行こう」とは考えていません。ただ「洞窟に行ったら楽しかった（報酬があった）」と日記につけただけです。

#### 2. 夜：夢の中での作戦会議 (Dreaming / Training)
エージェントは環境との対話を一時中断し、バッファに溜まった記憶を使って学習（夢）を見ます。

1.  **報酬モデルの学習**: ReplayBufferの記録を読み返し、「そういえば洞窟の映像を見たとき、高い報酬をもらったな」と学習します。これで、脳内で洞窟を想像するだけで「報酬が高い」と予測できるようになります。
2.  **Actorの練習**: 脳内シミュレーション（Imagination）を行います。
    *   Actor: 「右に行くと壁…左に行くと洞窟…」
    *   Critic: 「洞窟に行くと（報酬モデルによると）報酬が高いぞ！そっちに行け！」
    *   Actor: **「なるほど、現実に起きたら洞窟の方へ行くように体を動かそう（Policy更新）」**

#### 3. 次の日の昼：探索行動 (Action)
学習を終えたエージェントが再び環境（Craftium）に戻ります。
Actorのニューラルネットは「洞窟のような状態に近づく行動」の確率が高くなるように書き換わっています。そのため、エージェントは（意識的に探索しようとしているわけではなく）**過去に感動した場所へ、吸い寄せられるように**足を運びます。

#### 4. 飽きのメカニズム
何度も洞窟に行くと、世界モデルがその場所を学習し、予測できるようになります（Actual Error低下）。
するとLPM報酬が発生しなくなり、記憶の中の「洞窟＝高報酬」というデータが「洞窟＝低報酬」に上書きされていきます。Actorは洞窟への興味を失い、まだ見ぬ別の場所へと足を向けるようになります。
